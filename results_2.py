...
def  runres(filein, resolve, tol_pi):
	...
	def nextstor(s,d, gen):
		return min([max([grid_min_s,(1-delta_storage)*s - d + gen]),S_bar])
	...
	def return_pi(DS, S1, PI_bar):
		#if np.allclose(DS, zeta_storage*S_bar,tol_pi):
	...
	"""
	plt.savefig("{}_sim.png".format(filein, filein))
	...
	for i in range(0, og.grid_size_d):
		for j in range(0, og.grid_size_s):
			e = shock_index2[i,j]
			axrr_2[i,j].plot(grid_S, delta_func(e,grid_S))
			axrr_2[i,j].plot(grid_S, og.zeta_storage*np.ones(np.shape(grid_S))*S_bar)
			axrr_2[i,j].plot(grid_S, -og.zeta_storage*np.ones(np.shape(grid_S))*S_bar)

	f_2.tight_layout()

	plt.savefig("{}_deltas.png".format(filein, filein))

	f_3, axrr_3= plt.subplots(og.grid_size_s, og.grid_size_d)
	for i in range(0, og.grid_size_d):
		for j in range(0, og.grid_size_s):
			e = shock_index2[i,j]
			axrr_3[i,j].plot(grid_S, rho_func(e,grid_S))

	f_3.tight_layout()

	plt.savefig("{}_price.png".format(filein, filein))
	"""
