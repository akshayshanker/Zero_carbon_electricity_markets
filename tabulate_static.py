...
def latex_float(f):
    float_str = "{0:.3g}".format(np.float64(f))
    if "e" in float_str:
        ...
        for i_k in K_index:
            for i_s in S_index:
                ...
                if tab == True:
                    ...
                    "%.2f (%.2f)" % (results['mean_generation'],
                        results['var_generation']),
                        latex_float(results['mean_price']*1e-3 ) +
                         "(" + latex_float(results['var_price']*1e-3) + ")",
                        "%.2f (%.2f)" %
                        (results['mean_demand'],
                         results['var_demand']),
                        "%.2f (%.2f)" %
                ...
                table_ds[np.where(K_index == i_k),i_s] = results['var_demand']
                table_rs[np.where(K_index == i_k),i_s] = results['var_price']
                table_u[np.where(K_index == i_k),i_s] = results['WF']
                table_k[np.where(K_index == i_k),i_s] = results['K']
                table_s[np.where(K_index == i_k),i_s] = results["S_bar_star"]
                table_v[np.where(K_index == i_k),i_s] = VS_range[i_v]
                table_pi_s[np.where(K_index == i_k),i_s] = results['r_s_star']
                table_pi_k[np.where(K_index == i_k),i_s] = results['r_k_star']
...
if __name__ == '__main__':
    ...
    K_index = np.array([0,10])
    S_index = np.array([0,5, 10])
    V_index = np.array([2])
    ...
    for e in('3'): 
        ...
        K_index = np.array([5,10,20, 35,47])
        S_index = np.arange(48)
        V_index = np.array([2])
        ...
        for i_k in range(len(K_index)):
            axes[0].plot(table_s[i_k], table_pi_s[i_k], label='K = {:.2f}'\
                .format(K_range[K_index[i_k]],e), color=colors[i_k])
        ...
        colors = cmap_here(np.linspace(0,1,6))
        for i_k in range(len(K_index)):
            axes[1].plot(table_s[i_k], table_pi_k[i_k], label='K = {:.2f}'\
                .format(K_range[K_index[i_k]],e), color=colors[i_k])
        axes[1].set_xlabel('Storage')
        axes[1].legend()
        axes[1].spines['top'].set_visible(False)
        axes[1].spines['right'].set_visible(False)
        axes[1].set_ylabel('Generator returns')
            
        figure.savefig("Results/{}/{}/static/returns_r_s.png"\
            .format(model_name, sim_name))
        ...
        colors = cmap_here(np.linspace(0,1,6))
        for i_k in range(len(K_index)):
            axes[0].plot(table_s[i_k], table_rs[i_k]*1e-4, label = 'K = {:.2f}'\
                .format(K_range[K_index[i_k]],e), color = colors[i_k])
        axes[0].set_xlabel('Storage')
        axes[0].spines['top'].set_visible(False)
        axes[0].spines['right'].set_visible(False)
        ...
        for i_k in range(len(K_index)):
            axes[1].plot(table_s[i_k], table_ds[i_k], label = 'K = {:.2f}'\
            .format(K_range[K_index[i_k]],e), color = colors[i_k])
        axes[1].set_xlabel('Storage')
        axes[1].spines['top'].set_visible(False)
        axes[1].spines['right'].set_visible(False)
        ...
        figure.savefig("Results/{}/{}/static/returns_r_s_pvar.png"\
            .format(model_name, sim_name))
